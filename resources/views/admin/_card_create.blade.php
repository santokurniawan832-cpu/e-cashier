<form x-on:submit.prevent="btnSubmit()">
    <div class="grid gap-4 grid-cols-2 py-4 md:py-6">
        <div class="col-span-2 sm:col-span-1">
            ...
            <template x-if="errors.name">
                 <p class="mt-2.5 text-sm text-red-600"><span class="font-semibold" x-text="errors.name"></span> </p>
            </template>
        </div>
            <div class="col-span-2 sm:col-span-1">
            <label for="category" class="block mb-2.5 text-sm font-medium text-heading">Jumlah</label>
            <select id="category" x-model="product.quantity" name="quantity" class="rounded-lg block w-full px-3 py-2.5 bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand px-3 py-2.5 shadow-xs placeholder:text-body">
            ...
            <template x-if="errors.quantity">
                 <p class="mt-2.5 text-sm text-red-600"><span class="font-semibold" x-text="errors.quantity"></span> </p>
            </template>
        </div>
        <div class="col-span-2 sm:col-span-1">
           ...
           <template x-if="errors.price">
                 <p class="mt-2.5 text-sm text-red-600"><span class="font-semibold" x-text="errors.price"></span> </p>
            </template>
        </div>
        <div class="col-span-2 sm:col-span-1">
            <label for="category" class="block mb-2.5 text-sm font-medium text-heading">Ukuran</label>
            <select id="category" x-model="product.size" name="size" class="rounded-lg block w-full px-3 py-2.5 bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand px-3 py-2.5 shadow-xs placeholder:text-body">
            ...
            <template x-if="errors.size">
                 <p class="mt-2.5 text-sm text-red-600"><span class="font-semibold" x-text="errors.size"></span> </p>
            </template>
        </div>
        <div class="col-span-2">
            ...
            <textarea
                ...
                name="description"
                class="rounded-lg block bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full p-3.5 shadow-xs placeholder:text-body"
                placeholder="Ketikkan keterangan produk disini"></textarea>
            <template x-if="errors.description">
                <p class="mt-2.5 text-sm text-red-600"><span class="font-semibold" x-text="errors.description"></span> </p>
            </template>
        </div>
    </div>
    <div class="flex items-center space-x-4 pt-4 md:pt-6">
        <button 
            ...
            <span class="font-semibold" x-text="isProcess == true ? 'proses' : 'simpan'"> </span>
        </button>
        <button
            x-bind:disabled="isProcess"
            type="button"
            x-on:click="btnCancel"
            x-bind:class="{ 'bg-blue-400 cursor-not-allowed': isProcess, 'bg-blue-400 hover:bg-blue-600': !isProcess}"
